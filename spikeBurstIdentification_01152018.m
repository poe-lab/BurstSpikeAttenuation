function spikeBurstIdentification_01152018(cellParameters)
...
%% Define min/max difference in time between spikes to include in same burst:
BSIntervalMin = 0.002;
BSIntervalMax = 0.010; % time in seconds; up to 16 milliseconds allowed between spikes
...
%     waithandle= waitbar(0.2,['Loading spikes from ' fileName]);pause(0.2);
    ...
    tetrodeNum = tetrodeNum(nonZerosIndex)' + 1;   % Reduce to just 1 value
...
%     ADBitVolts = zeros(numChannels, 1);
    ...
    uniqueCells = unique(cellIdentity, 'rows'); % Vector for unique cells
    numOfCells = size(uniqueCells, 1); % Finds the number of cells in the data
    
    for m = 1:numOfCells
        %Find all spikes for matching unit:
        logicMatchCell = ismember(cellIdentity, uniqueCells(m,:), 'rows');
...
%         else
        ...
        cellIdentity = cellIdentity(~logicMatchCell,:); % Remove cell # from grouped data
        ...
        [placeCellLogic, placeCellidx] = ismember(uniqueCells(m,:), cellParameters(:,1:2), 'rows');
        if placeCellLogic
            ...
            if ~isempty(cellSpikeTS)
                ...
                diffTime = diff(cellSpikeTS) <= BSIntervalMax & diff(cellSpikeTS) > BSIntervalMin;
                ...
                if isempty(startBurst) || isempty(endBurst)
                    ...
                    maxAmp = zeros(numBursts, 1);
                    last2First = zeros(numBursts, 1);
                    for n = 1:numBursts
                        maxAmp(n) = max(cellPkAmp(startBurst(n):endBurst(n)));
                    ...
                    bursts.cellIdentity = [bursts.cellIdentity; (uniqueCells(m,:) .* ones(numBursts,1))];
...
bursts.cellIdentity = bursts.cellIdentity(IX,:);
...
save(fullfile(resultsFolder,['Bursts', filename, '.mat']), 'dataFolder', 'fileList', 'bursts', 'cellParameters');
