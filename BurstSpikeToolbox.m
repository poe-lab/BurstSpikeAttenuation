function varargout = BurstSpikeToolbox(varargin)
% BURSTSPIKETOOLBOX M-file for BurstSpikeToolbox.fig
%      BURSTSPIKETOOLBOX, by itself, creates a new BURSTSPIKETOOLBOX or raises the existing
%      singleton*.
%
%      H = BURSTSPIKETOOLBOX returns the handle to a new BURSTSPIKETOOLBOX or the handle to
%      the existing singleton*.
%
%      BURSTSPIKETOOLBOX('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BURSTSPIKETOOLBOX.M with the given input arguments.
%
%      BURSTSPIKETOOLBOX('Property','Value',...) creates a new BURSTSPIKETOOLBOX or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bsa_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to BurstSpikeToolbox_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help BurstSpikeToolbox

% Last Modified by GUIDE v2.5 05-Dec-2010 12:47:29
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @BurstSpikeToolbox_OpeningFcn, ...
                   'gui_OutputFcn',  @BurstSpikeToolbox_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before BurstSpikeToolbox is made visible.
function BurstSpikeToolbox_OpeningFcn(hObject, eventdata, handles, varargin)
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes BurstSpikeToolbox wait for user response (see UIRESUME)
% uiwait(handles.figure1);
function varargout = BurstSpikeToolbox_OutputFcn(hObject, eventdata, handles)
varargout{1} = handles.output;

% --- Mutually exclusive radio button group
function mutual_exclude(off1,off2)
set(off1,'Value',0);
set(off2,'Value',0);

function option1_radiobutton_Callback(hObject, eventdata, handles)
off1 = handles.option2_radiobutton;
off2 = handles.option3_radiobutton;
mutual_exclude(off1,off2);

function option2_radiobutton_Callback(hObject, eventdata, handles)
off1 = handles.option1_radiobutton;
off2 = handles.option3_radiobutton;
mutual_exclude(off1,off2);


function option3_radiobutton_Callback(hObject, eventdata, handles)
off1 = handles.option1_radiobutton;
off2 = handles.option2_radiobutton;
mutual_exclude(off1,off2);
...
% get analysis option
op1 = get(handles.option1_radiobutton,'value');
op2 = get(handles.option2_radiobutton,'value');
op3 = get(handles.option3_radiobutton,'value');

if op1 == 1
    option = 1;
    fprintf('Option 1: Original data\n');
elseif op2 == 1
    option = 2;
    fprintf('Option 2: Original & CPF data\n');
elseif op3 == 1
    option = 3;
    fprintf('Option 3: Sleep data\n');
end
