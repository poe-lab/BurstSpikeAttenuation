function results = compareLapSets_01152018(meanLast1stRatio, comboVector, set1, set2)
...
Set1_meanLast1stRatio = zeros(numOfCombos, 1);
Set2_meanLast1stRatio = zeros(numOfCombos, 1);
...
if sum(logicTarget) ~=0
    ...
    if sum(logicTarget) >=  6
        [~,results.tTestP] = ttest(Set1_meanLast1stRatio, Set2_meanLast1stRatio);
    end
else
    results = [];
end
